...
export const getBitcoinMarketData = async (): Promise<IBitcoinMarketChart> => {
    try {
        const options = { method: 'GET', headers: { 'x-cg-demo-api-key': import.meta.env.VITE_COINGECKO_API_KEY } };
        const response = await fetch('https://api.coingecko.com/api/v3/coins/bitcoin/market_chart?vs_currency=usd&days=1', options);
